...
def add_performances(imdb, theaters, dates):
    ...
    for theater in theaters:
        for date in dates:
            resource = url(f'/performances?imdbKey={imdb}&theatre={theater}&date={date}&time=19:30')
            try:
                # print("trying to add")
                r = requests.post(resource) # ERROR IS HERE
                print(r.text.strip())
